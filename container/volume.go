...
为每一个容器创造一个文件系统。
*/
func NewWorkSpaceV3(volume, imageName, containerName string) {
	...
	if volume != "" {
		...
		if length == 2 && volumeURLs[0] != "" && volumeURLs[1] != "" {
			MountVolumeV2(volumeURLs, containerName)
...
func CreateReadOnlyLayerV2(imageName string) error {
...
func CreateWriteLayerV2(containerName string) {
...
*/
func MountVolumeV2(volumeURLs []string, containerName string) error {
...
func CreateMountPointV2(containerName, imageName string) error {
	mntUrl := fmt.Sprintf(MntUrl, containerName)
...
func DeleteWorkSpaceV3(volume, containerName string) {
	if volume != "" {
		volumeURLs := strings.Split(volume, ":")
		length := len(volumeURLs)
		if length == 2 && volumeURLs[0] != "" && volumeURLs[1] != "" {
			DeleteMountPointWithVolumeV2(volumeURLs, containerName)
		} else {
			DeleteMountPointV2(containerName)
		}
	} else {
		DeleteMountPointV2(containerName)
	}
...
func DeleteMountPointV2(containerName string) error {
	mntURL := fmt.Sprintf(MntUrl, containerName)
...
func DeleteMountPointWithVolumeV2(volumeURLs []string, containerName string) error {
	mntURL := fmt.Sprintf(MntUrl, containerName)
...
func DeleteWriteLayerV2(containerName string) {
	writeURL := fmt.Sprintf(WriteLayerUrl, containerName)
...
func PathExistsV2(path string) (bool, error) {
	_, err := os.Stat(path)
	if err == nil {
		return true, nil
	}
